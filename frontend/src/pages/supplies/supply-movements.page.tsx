...
export function SupplyMovementsPage() {
  const [page, setPage] = useState(1);
  const { data: response, isLoading } = useQuery({
    queryKey: ['supply-movements', page],
    queryFn: () => suppliesApi.findAllMovements({ page: String(page), limit: '10' }),
  });
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Movimientos de Suministros</h1>
                ...
                {isLoading ? (
                  ...
                  movements.map((mov: any) => (
                    <tr key={mov.id} className="border-b hover:bg-muted/50">
                      <td className="px-4 py-3 font-medium">{mov.supplyItem?.nombre || mov.supplyItemId}</td>
